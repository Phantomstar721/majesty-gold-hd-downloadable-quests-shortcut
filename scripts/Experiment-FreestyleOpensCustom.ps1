...
function Find-FreestyleTextToken {
    ...
    for ($relative = 0; $relative -le ($Entry.Size - 28); $relative += 4) {
        ...
        if ($null -ne $textTokenOffset -and $hasImage5000) {
            return [pscustomobject]@{
...
function Patch-UiDataFile {
    ...
    $freestyle = Find-FreestyleTextToken $bytes $apdb
    if ($null -eq $freestyle) {
        return [pscustomobject]@{ Status = "Skipped"; Reason = "Could not find the Freestyle button text/action token."; Path = $Path }
    }
    ...
    $result = [pscustomobject]@{
        ...
        Path = $Path
        Rect = $freestyle.Rect
        Offset = ("0x{0:X}" -f ($freestyle.ElementOffset - $apdb.Offset))
    }
    ...
    Write-U32 $bytes $freestyle.TextValueOffset 17
...
Write-Host "Majesty Gold HD experiment: Freestyle opens Custom Quests"
...
foreach ($item in $results) {
    ...
    if ($item.Status -eq "Patched" -or $item.Status -eq "WouldPatch") {
        Write-Host ("{0}: {1} Freestyle text/action 77 -> 17 at {2} rect={3}" -f $name, $item.Status, $item.Offset, $item.Rect)
    } else {
