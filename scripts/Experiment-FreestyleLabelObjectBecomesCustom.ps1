...
function Find-FreestyleLabelObjectToken {
    ...
    for ($relative = 0; $relative -le ($Entry.Size - 28); $relative += 4) {
        ...
        for ($offset = $absolute + 28; $offset -le ($next - 8); $offset += 4) {
            ...
            if ($token -eq 7 -and ($value -eq 82 -or $value -eq 17)) {
        ...
        if ($null -ne $textValueOffset -and $null -ne $objectIdOffset -and $hasAction70) {
            return [pscustomobject]@{
...
function Patch-UiDataFile {
    ...
    $label = Find-FreestyleLabelObjectToken $bytes $apdb
    if ($null -eq $label) {
        return [pscustomobject]@{ Status = "Skipped"; Reason = "Could not find the stock Freestyle label object token."; Path = $Path }
    }
    ...
    Write-U32 $bytes $label.ObjectIdOffset 4034
...
Write-Host "Majesty Gold HD experiment: Freestyle label object becomes Custom Quests"
...
foreach ($item in $results) {
    ...
    if ($item.Status -eq "Patched" -or $item.Status -eq "WouldPatch") {
        Write-Host ("{0}: {1} Freestyle label object 5900 -> 4034 at {2} rect={3}" -f $name, $item.Status, $item.Offset, $item.Rect)
    } else {
